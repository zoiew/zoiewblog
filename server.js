        ...
        switch(RegExp.$1){
            ...
            case 'checkname':
                ...
                if (params.query && params.query.regname) {
                    ...
                    if(resText=="false"){
                        fs.writeFile(__dirname + name,JSON.stringify(data));
                    }
                }
                break;
            case 'weibo':
                ...
                break;
            case 'whf':
                var data = require('./data/wanghaifei.json') || [];
                var query = params.query;

                if(query){
                    if(query.type == "check"){
                        resText = JSON.stringify(data);
                    }else if(query.type == "reg"){
                        var postData = {
                            name:query.sender,
                            password:query.password
                        }
                        data.unshift(postData);
                        fs.writeFile(__dirname + '/data/wanghaifei.json',JSON.stringify(data));
                    }
                }
                break;
            case 'football':
